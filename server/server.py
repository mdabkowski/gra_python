...
class ServerThread(thread.Thread):
    def __init__(self, serversocket, gra):
        thread.Thread.__init__(self)
        self.socket = serversocket
        self.gra = gra
        self.header = 16
        print("Połączono z drugim serwerem")
    ...
    def sendPlayers(self):
        ...
        self.socket.sendall(bytes(f"{len(players_update):<{self.header}}", 'utf-8') + players_update)
    ...
    def receivePlayers(self):
        ...
        for player in players:
            ...
            if not any(gracz.name == player.name for gracz in self.gra.players):
                self.gra.players.append(Player(1, 1, str(len(self.gra.players)), player.name))
    ...
    def load(self, mapa):
        ...
        for row, tiles in enumerate(self.mapa):
            for col, tile in enumerate(tiles):
                ...
                if tile == 's':
                    self.skarby.append(Skarb(col, row, len(self.skarby)))
                if tile == "o":
                    self.przejscie.append(Przejscie(col, row))
        print("Mapa załadowana")

    def run(self):
        sconn, saddr = self.serv.accept()
        serverthread = ServerThread(sconn, self)
        serverthread.start()
        while True:
